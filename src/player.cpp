...
Player::Player() : card_num(Constants::initial_num_cards)
{
    ...
    red = {(Constants::WINDOW_WIDTH / 2.0f) - (color_picker_size),
           (Constants::WINDOW_HEIGHT / 2.0f) - (color_picker_size / 2.0f) + 100,
           color_picker_size, color_picker_size};
    green = {(Constants::WINDOW_WIDTH / 2.0f),
             (Constants::WINDOW_HEIGHT / 2.0f) - (color_picker_size / 2.0f) +
                 100,
             color_picker_size, color_picker_size};
    yellow = {(Constants::WINDOW_WIDTH / 2.0f) - (color_picker_size),
              (Constants::WINDOW_HEIGHT / 2.0f) + (color_picker_size / 2.0f) +
                  100,
              color_picker_size, color_picker_size};
    blue = {(Constants::WINDOW_WIDTH / 2.0f),
            (Constants::WINDOW_HEIGHT / 2.0f) + (color_picker_size / 2.0f) +
                100,
            color_picker_size, color_picker_size};
    ...
    rect = {(float)Constants::WINDOW_WIDTH / 2,
            ((float)Constants::WINDOW_HEIGHT - Constants::sprite_height) -
                Constants::border_offset,
            (Constants::sprite_width * 0.73f) * Constants::initial_num_cards,
            Constants::sprite_height};
    rect.x -= rect.width / 2;

    texture_rect = {Constants::sprite_offset_x +
                        (Constants::sprite_width * current_sprite.x),
                    Constants::sprite_offset_y +
                        (Constants::sprite_height * current_sprite.y),
...
void Player::draw()
{
    ...
    {
        texture_rect.x = Constants::sprite_offset_x +
                         (Constants::sprite_width * deck[i].pos.x);
        texture_rect.y =
            (Constants::sprite_offset_y - Constants::sprite_height) +
            (Constants::sprite_height * deck[i].pos.y),
        deck[i].rect.x =
            Lerp(deck[i].rect.x, rect.x + ((Constants::sprite_width / 1.5) * i),
                 0.2);
        deck[i].rect.y = Lerp(
            deck[i].rect.y,
            rect.y - ((Constants::sprite_height / 6) * deck[i].y_offset), 0.2);
...
void Player::create_card()
{
    ...
    card.rect = {
        ((float)Constants::WINDOW_WIDTH / 2) - (Constants::sprite_width * 2),
        ((float)Constants::WINDOW_HEIGHT / 2 + (Constants::sprite_width / 2)),
...
void Player::hover(Center &center)
{
    if (is_turn)
    {
        for (int i = 0; i < deck.size(); ++i)
        {
            collision_rect = {deck[i].rect.x, deck[i].rect.y,
                              (deck[i].rect.width) / 1.5f, deck[i].rect.height};
        ...
        if (deck.size() > 0)
        {
            collision_rect = {deck[last].rect.x, deck[last].rect.y,
                              deck[last].rect.width, deck[last].rect.height};
            if (CheckCollisionPointRec(GetMousePosition(), {collision_rect}))
            {
    ...
    else if (is_turn && pick_color == false)
    {
        ...
        for (int i = 0; i < Clamp(deck.size() - 1, 0, 1000); ++i)
        {
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
                CheckCollisionPointRec(GetMousePosition(), {collision_rect}))
            {
                if (center.random_x == deck[i].pos.x ||
                    center.random_y == deck[i].pos.y && center.random_y < 5)
                {
                    if (deck[i].pos.x == 10)
                    {
        ...
        // last card
        collision_rect = {deck[deck.size() - 1].rect.x,
                          deck[deck.size() - 1].rect.y,
                          deck[deck.size() - 1].rect.width,
                          deck[deck.size() - 1].rect.height};
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
            CheckCollisionPointRec(GetMousePosition(), {collision_rect}))
        {
            if (center.random_x == deck[deck.size() - 1].pos.x ||
                center.random_y == deck[deck.size() - 1].pos.y &&
                    center.random_y < 5)
            {
            ...
            else if (center.random_y == 5)
            {
                ...
                else if (deck[deck.size() - 1].pos.y == 5 &&
                         deck[deck.size() - 1].pos.x < 5 && wild_card_moves > 0)
                {
                    ...
                    is_turn = true;
                         ...
                         deck[deck.size() - 1].pos.x == 5 && normal_moves == 0)
                {
                    dish_wild(center, deck.size() - 1);
                    pick_color = true;
                    is_turn = true;
...
void Player::load_texture() { cards = LoadTexture("cards.png"); }
