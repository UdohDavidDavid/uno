...
Opp::Opp(ORT ort)
{
    ...
    {
        rect = {
            (Constants::WINDOW_WIDTH - Constants::sprite_width) -
                Constants::border_offset,
            Constants::WINDOW_HEIGHT / 2.0f,
            Constants::sprite_width,
            (Constants::sprite_height * 0.43f) * Constants::initial_num_cards,
...
void Opp::create_card()
{
    ...
    card.rect = {
        ...
        ((float)Constants::WINDOW_HEIGHT / 2 - (Constants::sprite_width)),
        Constants::sprite_width,
        Constants::sprite_height,
    };
}
...
void Opp::play(Center &center)
{
    if (possible_moves > 0)
    {
        if (center.random_y < 5)
        {
            for (int i = 0; i < deck.size(); ++i)
            {
                if (deck[i].pos.y == center.random_y ||
                    deck[i].pos.x == center.random_x && deck[i].pos.y != 5)
                {
                    if (deck[i].pos.x == 10)
                    {
                        dish_card(center, i);
                ...
                else if (deck[i].pos.y == 5 && deck[i].pos.x == 0 &&
                         wild_card_moves > 0)
                {
                    dish_wild(center, i);
                    return;
...
void Opp::update(Center &center)
{
    ...
    if (is_drawing_card)
    {
        clone = {currently_moving_card.rect.x, currently_moving_card.rect.y,
                 Constants::sprite_width, Constants::sprite_height};
        mov_x = Lerp(mov_x, center.rect.x, 0.2f);
        mov_y = Lerp(mov_y, center.rect.y, 0.2f);
        if (std::abs(center.rect.x - mov_x) < Constants::sprite_width / 2.0f &&
            std::abs(center.rect.y - mov_y) < Constants::sprite_height / 2.0f)
        {
            if (currently_moving_card.pos.y == 5)
            {
                center.random_x =
                    currently_moving_card.pos.x + check_possible_colors();
        ...
        clone.y = mov_y;
        texture_rect.x =
            Constants::sprite_offset_x +
            (Constants::sprite_width * currently_moving_card.pos.x);
        texture_rect.y =
            (Constants::sprite_offset_y - Constants::sprite_height) +
            (Constants::sprite_height * currently_moving_card.pos.y);
...
void Opp::draw()
{
    ...
    for (int i = 0; i < deck.size(); ++i)
    {
        ...
        if (this->orientation == TP)
        {
            deck[i].rect.x =
                Lerp(deck[i].rect.x,
                     rect.x + ((Constants::sprite_width / 1.5) * i), 0.2);
        ...
        else if (this->orientation == LH)
        {
            ...
            deck[i].rect.y =
                Lerp(deck[i].rect.y,
                     rect.y + ((Constants::sprite_height * 0.3) * i), 0.2);
        }
        else if (this->orientation == RH)
        {
            deck[i].rect.x = Lerp(deck[i].rect.x, rect.x, 0.2);
